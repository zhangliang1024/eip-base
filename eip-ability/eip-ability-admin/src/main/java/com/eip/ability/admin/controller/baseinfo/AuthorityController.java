...
public class AuthorityController {
    ...
    public ApiResult<List<DictResp>> list(@PathVariable("receiver_type") Integer receiverType) {
        ...
        if (receiverType == RECEIVER_TYPE_1) {
            ...
            if (CollectionUtil.isNotEmpty(users)) {
                result = users.stream().map(user -> DictResp.builder().label(user.getNickName()).value(user.getId()).build()).collect(toList());
            }
        } else if (receiverType == RECEIVER_TYPE_2) {
            final List<Role> roles = this.roleService.list(new DataScope());
            if (CollectionUtil.isNotEmpty(roles)) {
                result = roles.stream().map(role -> DictResp.builder().label(role.getName()).value(role.getId()).build()).collect(toList());
            }
        }
    ...
    @IgnoreAuthorize
    @GetMapping("/list")
    public ApiResult<DataScope> list() {
        return ApiResult.success(dataScopeService.getDataScopeById(7L));
    }
